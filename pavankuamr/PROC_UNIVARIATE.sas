/*PROC UNIVARIATE: */
/*This procedure produce simple descriptive statistics 
such as mean, std,median, percentiles*/
...
/*By: we can use the by stmt to obtain separate analysis
for each by group.*/
/*class: we can use class stmt to specify one or more 
variables that group the data into claissification levels.*/
/*id: the id stmt specifies one or more variables to 
identify the extreme observations.*/
/*var: the var stmt specifies the numeric variables to 
analyze the data*/
/*ouput: the o/p stmt saves statistics and by variables
in an o/p dataset.*/
...
/*Mode: The mode is the number that is repeated more often than any*/
/*other*/
...
/*Note: It will display top 5 & lowest 5 records for height & weight*/
/*variables*/
...
/*Note:It will display top 5 & lowest 5 records along with name of */
/*the persons for height & weight variables*/

/*No print: It wont' display the output*/
/*it will create the new datasets with required statistics*/
/*ods select ExtremeObs;*/
proc univariate data=gains noprint;
   var height weight;
   id name;
output out=new  mean=htmean wtmean;
output out=nes_stats mean=heightMean weightMean 
			std=heightstd weightstd
          	min=heightMin weightMin  
			max=heightmax weightMax; 
run;
...
/*ods pdf file="F:\Jagan Sas_class15\stats_univ.pdf";*/
ods select ExtremeObs;
