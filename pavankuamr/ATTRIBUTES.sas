...
/**/
/*Note: In dataset char data is left aligned and numeric*/
/*datais right aligned*/
...
/*2. LENGTH*/
/*specifies the  no.of bytes for storing vairable values
.*/
...
/*Excute this program and check the dataset*/
/*observe for name variable data will be truncated*/
data a1;
input id name $ sal loc $;
length name $10.;
datalines;
100 jaganmohan 34000 chennai
200 suryakiran 24000 chennai
300 vijayrsrik 34000 banga
400 niharikak 32000 delhi
;
run;
...
input id name $ sal loc $ ;
datalines;
100 jaganmohanreddy 40000 bang
200 suryakirankumar 50000 chen
300 vijayakumar 43000 che
;
run;
...
/*Note: Length stmt always should write before i/p or */
/*before set stmt or before variable. Otherwise when i/p*/
/*executes whatever the length is there(default 8) that */
/*will come into output dataset.*/

/*3. LABEL*/

/*Assign descriptive labels to variables. It gives
alias names to variables.*/

/*Note: Label can't change the variable name permanently*/
/*like rename*/
...
data a;
input id name $ sal loc $ age;
datalines;
100 jagan 40000 hyd 34
200 surya 50000 bang 30
300 vijaya 43000 che 30
;
run;

/*Execute this program check the o/p dataset*/
data a1;
...
/*Rename after excuting this please open the dataset 
and see the data*/
/*It will change the permanetnly*/
...
/**/
/*Note: If we write lable stmt after i/p stmt, output
dset will be in i/p order. But if we write before i/p,
output dset order should be in label order then whatever
variables are not there in label stmt those will come
in i/p order.*/
...
/*Informat is an instruction that sas uses to read the
data values into a variable.*/

/*Informats are usually specified in an i/p stmt.*/

/*syntax: infomat var-name informat;*/

/*1. Charcter informat  2. Numeric informat 
3. Date format*/
/*4. column binary informat 5. User-defined informat.*/

/*Character informats: Read a character data into character*/
/*variables*/
/**/
/*syntax: $informatw.  or $10.*/

/*Execute this prog and check the output dset*/
data a1;
/*input id name sal loc $;*/
input id name $10. sal loc $;
datalines;
100 jaganmohan 34000 chennai
200 suryakiran 24000 chennai
300 vijayrsrik 34000 banga
...
/*Numeric Informat*/
/*Read numeric data values from numeric variables*/

/*Syntax: informatw.d*/
/*ex: comma10. or comma10.2, dollar8. or dollar8.2;*/

data a2;
input id name $10. loc $ sal ;
datalines;
100 jaganmohan chennai 34000 
200 suryakiran chennai 54000
300 vijayrsrik banga 60000
400 niharikak delhi 58422
;
run;

/*Execute the program and check the o/p dataset*/
/*then comment first input stmt and uncomment the second i/p stmt*/
/*and execute, check the o/p dataset*/
data a2;
/*input id name $10. loc $ sal ;*/
input id name $10. loc $ sal comma6.;
datalines;
100 jaganmohan chennai 34,000 
200 suryakiran chennai 54,000
300 vijayrsrik banga 60,000
400 niharikak delhi 58,422
;
run;
...
/*informat sal comma.;*/
datalines;
100 jaganmohan hyd 34 m 20,000.5025
200 suryakiran bang 30 m 30,000.6065
300 vijayasrim che 23 f 40,000.0054
400 kumarisrin che 26 f 50,000.8544
500 raghureddy delhi 26 m 60,000.952
;
run;

data b7;
input id name $10. loc $ age gender $ sal sales;
/*informat sal dollar12.4 sales dollar8.;*/
informat sal comma. sales comma8.;
datalines;
100 jaganmohan hyd 34 m $20,000.5025 $34,000
200 suryakiran bang 30 m $30,000.6065 $34,000
300 vijayasrim che 23 f $40,000.0054 $24,000
400 kumarisrin che 26 f $50,000.8544 $64,000
500 raghureddy delhi 26 m $60,000.952 $4,000
;
run;

/*DATE formats*/
/*REads date values into variables representing time, dates
and datetimes.*/
...
/*anydtdte. if you doesn't know about date informat then we
can use this
/*time8   10:20:30(hrs:ms:seconds)*/
/*datetime. 30may2018:20:30:40*/

/*Execute this program and check the o/p dataset*/
data a;
...
100 jagan 40000 hyd 34 m 30mar83
200 surya 50000 bang 30 m 30mar89
300 vijaya 43000 che 23 f 29feb64
300 kumari 43000 che 26 f 31jan60
;
run;
...
/*IT will not read the :format dates it's cosider as 
time*/
data b6;
...
/*format doj datetime.;*/
datalines;
100 jagan 40000 hyd 34 m 30mar83 02jan60:10:40:50
...
/*DAys it will shows in -(minus)*/
data b6;
input id name $ sal loc $ age gender $ dob date7.;
informat dob date7.;
datalines;
100 jagan 40000 hyd 34 m 30mar53
200 surya 50000 bang 30 m 30mar89
...
/**/
/*user can create his own informats to read the data using PROC*/
/*format*/
...
data b6;
input id name $ sal loc $ age gender $ dept sahi.;
datalines;
...
informat m1 manimar. m2 manimar. m3 manimar.;
datalines;
100 jaga 50 60 70
200 surya 60 B 80
300 vijaya A B 60
...
/*format sal comma6.2;*/
datalines;
100 jaganmohan 20,000.50
200 suryakiran 30,000.60
300 vijayasrim 40,000.00
400 kumarisrin 50,000.85
500 raghureddy 60,000.98
;
run;
...
informat sal comma11.4;
/*format sal comma11.;*/
/*format sal comma11.2;*/
/*format sal comma11.3;*/
format sal comma11.4;
datalines;
100 jaganmohan 20,000.5056
200 suryakiran 30,000.6050
300 vijayasrim 40,000.0025
400 kumarisrin 50,000.8555
500 raghureddy 60,000.9825
;
run;


/*DOLLAR FORMATS*/
/*The below example read the in dollar format and write the*/
/*comma format*/
...
format sal dollar10.;
datalines;
100 jaganmohan $20,000
200 suryakiran $30,000
;
run;
...
datalines;
100 jaganmohan $20,000.00
200 suryakiran $30,000.55
300 vijayasrim $40,000.75
;
run;

/*The below it will read the data in dollar format and write into*/
/*euro format*/
data b10;
input id name $10. sal;
informat sal dollar10.2;
...
/*NOTE: ANYDTDTE IS NOT WORKING IN FORMAT STATEMENT*/

/*DATE7. FORMAT*/
data a;
input id name $ dob DATE7.;
FORMAT DOB DATE7.;
datalines;
100 jagan 30mar83
200 surya 30mar89
...
100 jagan 30mar1983
200 surya 30mar1989
300 vijaya 30apr1983
300 kumari 30jun1993
;
run;

/*Execute this program and check the log*/
data b5;
input id name $ dob :anydtdte.;
format dob anydtdte.;
datalines;
100 jagan 30mar83
200 surya 30mar1989
300 vijaya 30/04/1983
400 kumari 30-06-93
500 raghu 30/12/93
;
run;
...
data b6;
input id name $ dob DATETIME.;
format dob DATETIME.;
datalines;
100 jagan 30mar83:10:20:40
200 surya 30mar1989:10:20:40
300 vijaya 30mar1989:12:20:40
400 kumari 30mar1999:13:20:40
500 raghu 30mar2009:15:20:40
;
run;

/*The below example dob having multiple date types, so using Anydtdte*/
/*to read the data values*/
data b8;
input id name $ dob :anydtdte. doj datetime.;
/*informat dob anydtdte. doj datetime.;*/
format DOB DATE9. doj datetime.;
datalines;
100 jagan 30mar83 30mar83:10:40:50
200 surya 30mar1989 30mar89:12:40:50
300 vijaya 04/30/1983 30Apr83:22:40:50
400 kumari 30-06-93 30jun93:22:40:50
500 raghu 30/12/93 30dec1993:23:40:50
;
run;

/*The below example dob having multiple date types, so using anydtdte*/
/*informat to read the dob values*/
/*And apply the multiple formats*/
...
data raw1;
   input region $ state $ month monyy5. 
         headcnt expenses revenue; 
   format month monyy5. region sure.; 
   cards; 
Eastern  VA FEB78  10  7800 15500 
Southern FL MAR78   9  9800 13500 
Southern GA JAN78   5  2000  8000 
...
wrire with befor i/p or before set stmt otherwise when i/p stmt or 
set stmt execute whatever the length is there for variable that 
will come into o/p. and the o/pt dataset variable order also change
accordingly attrib stmt variables order that we cn change again
into required order in proc print.

but if we specify attrib stmt after i/p or set stmt dataset order
is i/p stmt order(we can specify like this when we are not using 
length in attrib stmt)
