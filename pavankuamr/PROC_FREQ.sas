...
/*BY: CALCULATE SEPARATE FREQUENCY OR CROSS TABULATION
TABLES FOR EACH BY GROUP*/
/*TABLES:  SPECIFIES FREQUENCY OR CROSS TABULATIONS TABLES
AND REQUEST TESTS;*/
/*OUTPUT: CREATE AN OUTPUT DATASET THAT CONTAIN
SPECIFICE DATA*/
/*WEIGHT: IDENTIFIES A VARIABLES WITH VALUES THAT 
...
proc freq data=raw;
tables region state region*state/out=test;
run;
...
ods rtf file='F:\JAGAN Sas_class\bhartath\freq.rtf';
proc freq data=raw3 page;
by region;
tables state/out=freq_1;
weight expenses;
...
/*Sparse: This option include zero cell frequencies in the 
o/p dset*/

/*After executing this you can compare the t5 & t6 dataset*/
proc freq data=raw3;
tables region/out=t1 ;
tables region/out=t11 nocum nopercent;
tables state/out=t2;
tables month/out=t3;
...
proc freq data=raw3;
tables region*state/out=t6 NOCUM NOPERCENT
				NOCOL
					NOROW;
weight expenses;
run;
...
proc freq data=raw;
tables region state 
		region*state/plots=freqplot (type=dotplot);
weight expenses;
run;
ods graphics off;
...
Eastern  VA MAR78  11  8200 16600 
Plains   NM MAR78   2  1350   500 
Southern . JAN78  10  8000 10000 
Northern NY FEB78   4  3000  4000 
. GA FEB78   7  1200  6000 
; 
run; 

proc freq data=raw2 ;
tables region / out=t;
tables region / out=t1 missing;
tables region*state/ out =tt missing;
run;
