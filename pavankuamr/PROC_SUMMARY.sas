...
tools to calculate descriptive statistics for variables
across all observation and within group of observation.*/
...
of data*/
/*ways: specify the no.of ways to make unique 
comnination of class variables*/
...
/*In proc summary we need to write var stmt otherwise it will not*/
/*give any statistics and create the new dataset as tt with n mean*/
/*max mis std*/
proc summary data=gains print ;
var age;
output out=tt;
...
proc summary data=gains n min range print;
var age;
run; 

proc means data=gains n min range;
var age;
run; 

proc summary data=gains n mean range;
var age;
output out=new;
run; 
...
/*Note: execute the means & Summary procedure both will give the*/
/*same results*/
...
/*in output dataset gives maxheight maxweight and minht and minwt*/
...
proc summary data=gains maxdec=1 fw=6 nonobs missing
		print;
var height weight;
class sex school;
...
/*Nonobs: It will suppress the observation in output*/
/*maxdec: It will print the no of decimals after . in output*/
/*fw: It will print the no of bytes in cell values*/
/*missing: It will include the missing values in calulation time*/
...
output out=new3 max=maxheight maxwieght min=minht minwt
	...
	minid(height(name) weight(name))=short lowweight;
	title 'one way and two way calculations';
	title2 'this process generated by 1130 batch team';
	footnote 'generated by swati';
