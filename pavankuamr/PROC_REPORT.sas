...
Run;
/*PROC REPORT */
/*The REPORT procedure (PROC REPORT) combines features of*/
/*the PRINT, TABULATE, and MEANS procedures, along with */
/*features of the DATA step, in a single report-writing */
/*tool that can be to produce a variety of reports.*/
/**/
/*It is useful to print all or selected variables, Find*/
/*outs descriptive statistics and computes new variables.*/
...
/*COLUMN :Select the variable which you want to prints in*/
/*the report. Default it selects all variables. */
/**/
/*DEFINE :It is useful to define variables.*/
/*Like applying attributes, alignment of columns and data,*/
/*applying styles etc…*/
...
/*It is useful to print statistical calculations on each*/
/*group or order variable wise for analysis variables. Or*/
/*produces a default summary at a break (a change in the */
/*value of a GROUP or ORDER variable).*/
/*Location controls the placement of the break lines, */
/*where location is */
/*AFTER places the break lines immediately after the last*/
/*row of each set(group) of rowsthat have the same value*/
/*for the break variable.*/
/*BEFORE places the break lines immediately before the */
/*first row of each set(group) of rows that have the same*/
/*value for the break variable.*/
/**/
/*BREAK AFTER statement options*/
/*OL inserts a line of hyphens (-) above each value that*/
/*appears in the summary line.*/
/*UL inserts a line of hyphens (-) under each value that*/
/*appears in the summary line.*/
/*DOL inserts double line of hyphens (-) above each value*/
/*that appears in the summary line.*/
/*DUL inserts double line of hyphens (-) under each value*/
/*that appears in the summary line.*/
/*SKIP writes a blank line for the last break line.*/
/*SUMMARIZE writes a summary line in each group of break*/
/*lines. */
/**/
/**/
/*RBREAK AFTER statement*/
/*produces a default summary at the beginning or end of a*/
/*report.*/
/*Syntax:- RBREAK location </option(s)>;*/
/*Location controls the placement of the break lines and*/
/*is either AFTER places the break lines at the end of */
/*the report.*/
/*BEFORE places the break lines at the beginning of the*/
/*report*/
...
/*BOX: Report the data within box in output window, 
default print without box.*/
Proc report data= raw out=ds2 box;
Run;

/*nowd/nowindows*/
/*Suppress to printing of proc report window and 
prints report in*/
/*output or result viewer window.*/
/*Select the interactive report window or the non 
...
/*BYPAGENO=number:*/
/*If a BY statement is present, specifies the listing 
page number
...
/*Default is center, when we specify nocenter it prints
left aligned.*/
Proc report data=raw nowd box bypageno=2  nocenter ;
By region;
Run;

/*HEAD SKIP: Insert space b/w column name and data in
output window.*/
Proc report data= raw nowd headskip;
Run;

/*HEAD LINE: Insert line b/w column names and data
in o/p window.*/
Proc report data= raw nowd headskip headline;
Run;
...
/*NOHEADER: suppresses column headings, including 
...
/**/
/*Style={background=color} - Applying back ground */
/*color for column name and data.*/
/**/
/*Style={foreground=color} - Applying foreground */
/*color for column name and data.*/
/**/
...
/**/
/*Style={font_size=size of font} – Increase/decrease */
/*font_size.*/
/**/
/*Applying alignment for variables and data.*/
/*Center – Center align for data values*/
/*Left – Left align for data values, default character */
/*variables values are left aligned.*/
/*Right – Right align for data values, default numeric */
/*variables values are Right aligned.*/
/*style(column)={just=center | left | right} – Alignment */
/*for data*/
/*values*/
/*style(header)={just=center | left | right} – Alignment */
/*for column*/
...
Define month/  width=6 left;
Define expenses/ 'Emp expenses' format=dollar10. center;
Title  'Emp Demographic Info ';
Title2 'Stansys Software Solutions';
Footnote 'This is a system generated mail please dont 
respond to this ';
Run;
...
proc report data=raw nowd;
columns region state expenses revenue headcnt;
...
which yields the same order that*/
/*PROC SORT would yield. This order is operating 
environment dependent. This sort sequence*/
/*is particularly useful for displaying dates 
chronologically.*/



/*ORDER – Sort the rows and prints on each group wise.*/
/*GROUP – Consolidate observations into groups and 
removes if entire
observation is duplicates.*/
/*ANALYSIS – Calculate a statistics, default for 
numeric variables.*/
...
proc report data=raw out=ni nowd;
columns region state month expenses;
define region/order;/*it will sort in asceding order*/
define state/order order=internal descending;
define expenses/analysis sum format=dollar10.2;
run;
...
compute after region;
      line 'sarath wants subtotal for ' region $reg. 
'is '
            expenses.sum dollar9.2 '.';
   endcomp;

   compute after;
       line 'Total for all region is: '
            expenses.sum dollar9.2 '.';
   endcomp;
run;
...
/*Note: If we use order=formatted with format, 
then the data will be*/
/*display in ascending order based on the format(new) 
values*/
...
footnote 'Mukesh genarated the report';
run;

proc report data=raw nowd /*box*/;
columns region state expenses expenses=smax expenses=smin;
define region/ORDER 'region name' descending;
define state/order;
define expenses/analysis sum format=dollar10.2;
define smin/'salesmin' analysis min ;
define smax/'salesmax' analysis max ;
...
compute after;
	...
	line ' ';
	line @10 60*'*';
	line @10 'the total expenses from:'
...
proc report data=raw nowd;
columns region state expenses,(sum min max mean);
define region/group ;
define state/group;
...
/*options ls=120;*/
proc report data=raw nowd;
columns region state expenses=expenses1 expenses,
(sum n min max)
		revenue,(sum mean min max std);
define region/group ;
/*define state/group;*/
define expenses/format=dollar10.2;
define expenses1/'newexp';
break after region/summarize ol skip;
rbreak after /summarize ;
run;

proc report data=raw box nowd;
/*columns region state expenses,(mean sum) n ;*/
columns region state expenses n ;
define region/group;
define state/group;
/*define expenses/analysis mean format=dollar10.2;*/
rbreak after /summarize ;
run;
...
proc report data=raw  nowd;
columns region state month expenses;
define region/style(header)={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial}
			style(column)={background=lightgreen foreground=Brown font_weight=bold
		font_size=3 font_face=Arial};
define month/across style(header)={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial}
			style(column)={background=lightgreen foreground=Brown font_weight=bold
		font_size=3 font_face=Arial} ;
define expenses/analysis sum width=13  format=dollar10.2
		style(header)={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial}
			style(column)={background=lightgreen foreground=Brown font_weight=bold
		font_size=3 font_face=Arial};
label Region='Region_Name' month='Month Name' state='State Name';
rbreak after /summarize style={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial} ;
/*rbreak before /summarize  ;*/
run;
...
define bonus/computed style(header)={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial}
			style(column)={background=red foreground=Brown font_weight=bold
		font_size=3 font_face=Arial};
...
ods html file='F:\Jagan Sas_class3\data\report.html';
proc report data=sashelp.class box nowd;
...
rbreak after/summarize
	style(summary)={background=orange foreground=Brown font_weight=bold
		font_size=3 font_face=Arial};
...
compute new_wt;
	...
	if age.sum in (13,14) then new_wt=weight.sum+15;
	else
...
Proc format;
Value $gen 
'M'='C:\Users\Public\Pictures\Sample Pictures\Jellyfish.jpg'
 'F'= 'C:\Users\Public\Pictures\Sample Pictures\Penguins.jpg';
Run;
...
columns name sex age height weight new_ht new_wt diff;
define sex/order 
	style(column)={preimage=$gen. };
define new_ht/computed style={backgroundcolor=ht.};
define new_wt/computed;
define diff/computed;
by sex;
compute new_ht;
