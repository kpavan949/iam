...
/*determine the categories that proc tab uses to calculate the */
/*statistics*/
/**/
...
regions*/
proc tabulate data=raw format = dollar8.2;
class region state month;
var expenses;
...
/*inthis example pagedimension,row dimension & column*/
/*dimension region is page dime, state is row dimension, */
/*expenses is column dimension*/
Proc Tabulate Data= raw Format=8.1; 
class region state month;
var expenses;
Table region, state all, sum*expenses='sumexp'
(month all); 
Table region, state all, pctsum*expenses=' '*
(month all); 
Table region, state all, n*expenses=' '*
(month all); 
Table region, state all, nmiss*expenses=' '*
(month all); 
Table region, state all, range*expenses=' '*(month all); 
Table region, state all, stddev*expenses=' '*
(month all); 
Table region, state all, css*expenses=' '*(month all); 
Table region, state all, skewness*expenses=' '*
(month all); 
...
table region*(state all='subtotal'*f=dollar10.2) 
all='total',
	month*(sum='total sales' n='no of records' mean=
'avg' min='minimum sales' max='max sales')*sales;
	label sales='REGIONS SALES';
run;

/*Applying styles(colors) using define statement*/
/*Style={background=color} - Applying back ground color
for 
column name and data.*/
/*Style={foreground=color} - Applying foreground color
for column 
name and data.*/
...
/*Printmiss:IT is specifies that all possible 
combination
level of the class variables. */

/*Misstext:Instead of missing values it will be print 
the defined*/
...
all='total',
	month*(sum='total sales' n='no of records' mean='avg' 
			min='minimum sales' max='max sales')*
sales/misstext='NONE'
				printmiss box='STANSYS SAS REPORT';
	label sales='REGIONS SALES';
	title 'This is sales';
...
/*In this example we are using pagedimension, row 
...
region*(state all='subtotal'*f=dollar10.2) all='total',
	month*(sum='total sales' n='no of records'
mean='avg' 
			min='minimum sales' max='max sales')*
sales/misstext='0000'
			box='SalesREPORT';
...
proc tabulate data=raw 
	Style={background=blue foreground=brown 
font_weight=bold font_face=Algerian};
class region state month/Style={background=lightgreen 
foreground=red font_weight=bold};
var sales;
table month, region*(state all='subtotal'*f=dollar10.2)
all='total',
	month*(sum='total sales' n='no of records' mean='avg' 
			min='minimum sales' max='max sales')*
sales/misstext='0000' box='SalesREPORT/two';
	label sales='REGIONS SALES';
	title 'This is sales';
	title2 "This report generated by FEB month";
	footnote 'This is monthly report';
...
proc tabulate data=raw 	out=test2
	Style={background=blue foreground=brown font_weight=
bold font_face=Algerian};
class region/Style={background=lightgreen foreground=
red font_weight=bold font_size=2 font_face=Arial};

class state/Style={background=SKYBLUE foreground=blue 
font_weight=bold font_size=5 font_face=Batang};

class month/Style={background=PINK foreground=black 
font_weight=bold font_size=5 font_face=calibri};

Classlev region state month/Style={background=PINK 
foreground=BROWN};
var sales;
table month,
	region*(state all='subtotal'*f=dollar10.2) 
all='total',
	month*(sum='total sales' n='no of records' mean='avg' 
			min='minimum sales' max='max sales')*
sales/misstext='0000' box='SalesREPORT/two';
...
ods pdf file="F:\Jagan Sas_class3\data\tabulate1.pdf";

proc tabulate data=raw OUT=TES
	Style={background=yellow foreground=red 
font_weight=bold font_face=Algerian}
	;
class region state month/Style={background=lightgreen 
foreground=black font_weight=bold};
Classlev region state month/Style={background=lime 
foreground=BROWN};
var sales/Style={background=orange foreground=BROWN};
keylabel all = 'Total';
keyword all/Style={background=orange foreground=BROWN};
keyword sum n mean/Style={background=skyblue foreground=BROWN};
table month , region*(state all='subtotal'
*[style=[backgroundcolor=red]]) all='total'*[style=
[backgroundcolor=brown]],
	month*(sum='total sales' n='no of records' mean='avg' 
			)*sales/
	misstext='0000' box=[label="Region by Division by 
Type" style=[background=lime fontstyle=italic]];
	label sales='REGIONS SALES';
	title 'This is sales';
	title2 "This report generated by FEB month";
	footnote 'This is monthly report';
/*	It will extract the southern data*/
...
proc tabulate data=raw
	Style={background=color. foreground=brown 
font_weight=bold 
	font_face=calibri};
class region state month;
var expenses;
...
/*IT will create the separate section by regionwise*/
Proc Tabulate Data= raw2 Format=8.1; 
class region state month;
var expenses revenue;
by region;
Table region*state all,
